...
const csrf: RequestHandler = (request, _response, next) => { const origin=request.get("origin"); if(origin && origin !== process.env.WEB_ORIGIN && origin !== "http://localhost:3000") { next(new AppError(403,"CSRF_REJECTED","Request origin is not allowed.")); return; } next(); };
