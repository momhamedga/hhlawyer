...
beforeAll(async () => { for (const role of ["ADMIN", "LAWYER", "STAFF"] as const) { const user = await createUser(role, role.toLowerCase()); if (role === "ADMIN") actorId = user.id; const agent = request.agent(app); expect((await agent.post("/api/v1/auth/login").set("Origin", "http://localhost:3000").send({ email: user.email, password })).status).toBe(200); agents[role] = agent; } }, 30_000);
