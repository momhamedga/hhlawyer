...
export function getAdminContacts(f:ContactFilters,signal?:AbortSignal){const p=new URLSearchParams();Object.entries(f).forEach(([k,v])=>{if(v)p.set(k,String(v));});return apiFetch<AdminContactListResponse>(`/admin/contacts?${p}`,{credentials:"include",signal});}
export function getAdminContact(id:string,signal?:AbortSignal){return apiFetch<AdminContactDetail>(`/admin/contacts/${id}`,{credentials:"include",signal});}
