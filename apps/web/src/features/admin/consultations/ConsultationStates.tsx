...
export function ConsultationListSkeleton({ locale }: { locale: Locale }) {
...
export function ConsultationDetailSkeleton({ locale }: { locale: Locale }) {
  const copy = consultationsContent[locale];
...
export function ConsultationEmpty({ filtered, locale, onClear }: { filtered: boolean; locale: Locale; onClear: () => void }) {
  const copy = consultationsContent[locale];
  return <div className={styles.stateWrap}><CalendarClock aria-hidden="true" size={24} /><AdminDataState className={styles.state} description={filtered ? copy.filteredEmptyDescription : copy.emptyDescription} title={filtered ? copy.filteredEmptyTitle : copy.emptyTitle} />{filtered ? <Button onClick={onClear} variant="outline">{copy.clearFilters}</Button> : null}</div>;
}

export function ConsultationError({ forbidden = false, locale, notFound = false, onRetry }: { forbidden?: boolean; locale: Locale; notFound?: boolean; onRetry: () => void }) {
  const copy = consultationsContent[locale];
  ...
  return <div className={styles.stateWrap}><AlertTriangle aria-hidden="true" size={24} /><AdminDataState className={styles.state} description={description} title={title} tone="error" />{!forbidden && !notFound ? <Button onClick={onRetry} variant="outline"><RefreshCw aria-hidden="true" size={16} />{copy.retry}</Button> : null}</div>;
}
