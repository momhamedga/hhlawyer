...
test("Contact is bilingual, editorial, light-safe, and exposes real direct contact links", async ({ page }) => {
  ...
  for (const locale of ["ar", "en"] as const) {
    ...
    const contactInformation = page.getByTestId("contact-information");
    await expect(contactInformation.locator('a[href="tel:+97126261565"]')).toBeVisible();
    await expect(contactInformation.locator('a[href="mailto:info@hhlawyer.ae"]')).toBeVisible();
    await expect(page.getByTestId("contact-value-phone")).toHaveText("026261565");
    await expect(page.getByTestId("contact-value-email")).toHaveText("info@hhlawyer.ae");
    await expect(contactInformation).not.toContainText("0502001797");
    await expect(contactInformation).not.toContainText("info@hussein.ae");
    const footer = page.locator('[data-public-footer="true"]');
    await expect(footer.locator('a[href="tel:+97126261565"]')).toContainText("026261565");
    await expect(footer.locator('a[href="mailto:info@hhlawyer.ae"]')).toContainText("info@hhlawyer.ae");
    await expect(footer).not.toContainText("0502001797");
    await expect(footer).not.toContainText("info@hussein.ae");
